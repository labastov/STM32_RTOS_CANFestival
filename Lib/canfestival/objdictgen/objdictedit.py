        ...
        def Bind(self, event, handler, source=None, id=wx.ID_ANY, id2=wx.ID_ANY):
            if event == HtmlWindowUrlClick:
                ...
                wx.html.HtmlWindow.Bind(
                    event, handler, source=source, id=id, id2=id2)
...
class objdictedit(wx.Frame, NodeEditorTemplate):
    ...
    def _init_coll_FileMenu_Items(self, parent):
        parent.Append(helpString='New\tCTRL+N', id=wx.ID_NEW,
                      kind=wx.ITEM_NORMAL)
        parent.Append(helpString='Open\tCTRL+O', id=wx.ID_OPEN,
                      kind=wx.ITEM_NORMAL)
        parent.Append(helpString='Close\tCTRL+W', id=wx.ID_CLOSE,
                      kind=wx.ITEM_NORMAL)
        parent.AppendSeparator()
        parent.Append(helpString='Save\tCTRL+S', id=wx.ID_SAVE,
                      kind=wx.ITEM_NORMAL)
        parent.Append(helpString='Save As...\tALT+S', id=wx.ID_SAVEAS,
                      kind=wx.ITEM_NORMAL)
        parent.AppendSeparator()
        parent.Append(ID_OBJDICTEDITFILEMENUIMPORTEDS, 'Import EDS file')
        parent.Append(ID_OBJDICTEDITFILEMENUEXPORTEDS, 'Export to EDS file')
        parent.Append(ID_OBJDICTEDITFILEMENUEXPORTC, 'Build Dictionary\tCTRL+B')
        parent.AppendSeparator()
        parent.Append(helpString='Exit', id=wx.ID_EXIT,
                      kind=wx.ITEM_NORMAL)
    ...
    def _init_coll_EditMenu_Items(self, parent):
        parent.Append(helpString='Refresh\tCTRL+R', id=wx.ID_REFRESH,
                      kind=wx.ITEM_NORMAL)
        parent.AppendSeparator()
        parent.Append(helpString='Undo\tCTRL+Z', id=wx.ID_UNDO,
                      kind=wx.ITEM_NORMAL)
        parent.Append(helpString='Redo\tCTRL+Y', id=wx.ID_REDO,
                      kind=wx.ITEM_NORMAL)
        parent.AppendSeparator()
        parent.Append(ID_OBJDICTEDITEDITMENUNODEINFOS, 'Node infos')
        parent.Append(ID_OBJDICTEDITEDITMENUDS301PROFILE, 'DS-301 Profile')
        parent.Append(ID_OBJDICTEDITEDITMENUDS302PROFILE, 'DS-302 Profile')
        parent.Append(ID_OBJDICTEDITEDITMENUOTHERPROFILE, 'Other Profile')
    ...
    def _init_coll_AddMenu_Items(self, parent):
        parent.Append(ID_OBJDICTEDITADDMENUSDOSERVER, 'SDO Server')
        parent.Append(ID_OBJDICTEDITADDMENUSDOCLIENT, 'SDO Client')
        parent.Append(ID_OBJDICTEDITADDMENUPDOTRANSMIT, 'PDO Transmit')
        parent.Append(ID_OBJDICTEDITADDMENUPDORECEIVE, 'PDO Receive')
        parent.Append(ID_OBJDICTEDITADDMENUMAPVARIABLE,'Map Variable')
        parent.Append(ID_OBJDICTEDITADDMENUUSERTYPE,'User Type')
    ...
    def _init_coll_HelpMenu_Items(self, parent):
        parent.Append(helpString='DS-301 Standard\tF1', id=wx.ID_HELP,
                      kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnHelpDS301Menu, id=wx.ID_HELP)
        # parent.Append(helpString='CAN Festival Docs\tF2', id=wx.ID_HELP_CONTEXT,
        #               kind=wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.OnHelpCANFestivalMenu,
                  id=wx.ID_HELP_CONTEXT)
        if Html_Window and self.ModeSolo:
            parent.Append(helpString='About', id=wx.ID_ABOUT,
                          kind=wx.ITEM_NORMAL)
    ...
    def __init__(self, parent, manager=None, filesOpen=[]):
        ...
        icon = wx.Icon(os.path.join(ScriptDirectory,
                                    "networkedit.ico"), wx.BITMAP_TYPE_ICO)
        self.SetIcon(icon)
            ...
            if result:
                ...
                if isinstance(result, (StringType, UnicodeType)):
                    message = wx.MessageDialog(
                        self, result, _("ERROR"), wx.OK | wx.ICON_ERROR)
                    message.ShowModal()
                    message.Destroy()
        if not find_index:
    ...
    def OnHelpCANFestivalMenu(self, event):
        ...
        if wx.Platform == '__WXMSW__':
            ...
            if(os.path.isfile(readerexepath)):
                os.spawnl(os.P_DETACH, readerexepath, "AcroRd32.exe", '"%s"' %
                          os.path.join(ScriptDirectory, "doc", "manual_en.pdf"))
            else:
                message = wx.MessageDialog(self, _(
                    "Check if Acrobat Reader is correctly installed on your computer"), _("ERROR"), wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()
        else:
            try:
                ...
                message = wx.MessageDialog(self, _(
                    "Check if xpdf is correctly installed on your computer"), _("ERROR"), wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()

    def OnAboutMenu(self, event):
        self.OpenHtmlFrame(_("About CAN Festival"), os.path.join(
            ScriptDirectory, "doc/about.html"), wx.Size(500, 450))
    ...
    def OnCloseFrame(self, event):
        ...
        if not self.ModeSolo:
            ...
            dialog = wx.MessageDialog(self, _("There are changes, do you want to save?"),  _(
...
# -------------------------------------------------------------------------------
    ...
    def RefreshTitle(self):
        if self.FileOpened.GetPageCount() > 0:
            self.SetTitle(_("Objdictedit - %s") %
                          self.Manager.GetCurrentFilename())
        else:
...
# -------------------------------------------------------------------------------
    ...
    def OnNewMenu(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            result = self.Manager.CreateNewNode(
                name, id, nodetype, description, profile, filepath, NMT, options)
            if isinstance(result, (IntType, LongType)):
                new_editingpanel = EditingPanel(
                    self.FileOpened, self, self.Manager)
                new_editingpanel.SetIndex(result)
    ...
    def OnOpenMenu(self, event):
        ...
        dialog = wx.FileDialog(self, _("Choose a file"), directory, "",  _(
            "OD files (*.od)|*.od|All files|*.*"), wx.FD_OPEN | wx.FD_CHANGE_DIR)
        if dialog.ShowModal() == wx.ID_OK:
    ...
    def SaveAs(self):
        ...
        dialog = wx.FileDialog(self, _("Choose a file"), directory, filename,  _(
            "OD files (*.od)|*.od|All files|*.*"), wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR)
        if dialog.ShowModal() == wx.ID_OK:
            filepath = dialog.GetPath()
            if os.path.isdir(os.path.dirname(filepath)):
                ...
                message = wx.MessageDialog(self, _("%s is not a valid folder!") % os.path.dirname(
...
# -------------------------------------------------------------------------------
    ...
    def OnImportEDSMenu(self, event):
        dialog = wx.FileDialog(self, _("Choose a file"), os.getcwd(), "",  _(
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if os.path.isfile(filepath):
                ...
                if isinstance(result, (IntType, LongType)):
                    ...
                    self.RefreshProfileMenu()
                    self.RefreshMainMenu()
                    message = wx.MessageDialog(self, _("Import successful"), _(
                ...
                message = wx.MessageDialog(self, _(
                    "\"%s\" is not a valid file!") % filepath, _("Error"), wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()
        dialog.Destroy()

    def OnExportEDSMenu(self, event):
        dialog = wx.FileDialog(self, _("Choose a file"), os.getcwd(), self.Manager.GetCurrentNodeInfos(
        )[0], _("EDS files (*.eds)|*.eds|All files|*.*"), wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR)
        if dialog.ShowModal() == wx.ID_OK:
            filepath = dialog.GetPath()
            if os.path.isdir(os.path.dirname(filepath)):
                ...
                if not result:
                    message = wx.MessageDialog(self, _("Export successful"), _(
                        "Information"), wx.OK | wx.ICON_INFORMATION)
                    message.ShowModal()
                    message.Destroy()
                else:
    ...
    def OnExportCMenu(self, event):
        dialog = wx.FileDialog(self, _("Choose a file"), os.getcwd(), self.Manager.GetCurrentNodeInfos()[0],  _(
            "CANFestival C files (*.c)|*.c|All files|*.*"), wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR)
        if dialog.ShowModal() == wx.ID_OK:
            filepath = dialog.GetPath()
            if os.path.isdir(os.path.dirname(filepath)):
                path, extend = os.path.splitext(filepath)
...
def Display_Error_Dialog(e_value):
    message = wx.MessageDialog(
        None, str(e_value), _("Error"), wx.OK | wx.ICON_ERROR)
    message.ShowModal()
    message.Destroy()
    ...
    def handle_exception(e_type, e_value, e_traceback):
        ...
        if str(e_value).startswith("!!!"):
            ...
            if result:
                ...
                output = open(path+os.sep+"bug_report_" +
                              info['date'].replace(':', '-').replace(' ', '_')+".txt", 'w')
                lst = info.keys()
                lst.sort()
                for a in lst:
                    output.write(a+":\n"+str(info[a])+"\n\n")
