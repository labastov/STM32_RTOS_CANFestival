...
class MapVariableDialog(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.Bind(wx.EVT_BUTTON, self.OnOK,
                  id=self.ButtonSizer.GetChildren()[0].GetId())
...
class UserTypeDialog(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.ButtonSizer = self.CreateButtonSizer(wx.OK|wx.CANCEL)
        self.Bind(wx.EVT_BUTTON, self.OnOK, id=self.ButtonSizer.GetChildren()[0].GetId())
...
class NodeInfosDialog(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        wx.Dialog.__init__(self, id=ID_NODEINFOSDIALOG,
              name='NodeInfosDialog', parent=prnt, pos=wx.Point(376, 323),
    ...
    def __init__(self, parent):
        ...
        # я не полял зачем это добавили, глупость
    ...
    def OnOK(self, event):
        ...
        if message != "":
            try:
                ...
                if nodeid > 127:
                    nodeid=0
                    message = _("Node ID must be between 0 and 127!")
            except:
    ...
    def GetValues(self):
        ...
        if self.NodeID.GetValue() != "":
            try:
                ...
                message = _("ERR")
        if message != "":
            message = wx.MessageDialog(self, _("Node Id isn't valid. Node Id must be number between 0 and 127!\n\n Node Id Set 0x00"), _("Error"), wx.OK | wx.ICON_ERROR)
            message.ShowModal()
            message.Destroy()
        type = NODE_TYPES_DICT[self.Type.GetStringSelection()]
        description = self.Description.GetValue()
