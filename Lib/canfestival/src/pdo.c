...
UNS8 buildPDO(CO_Data *d, UNS8 numPdo, Message *pdo) {
  ...
  pdo->cob_id =
      (UNS16)UNS16_LE(*(UNS32 *)TPDO_com->pSubindex[1].pObject & 0x7FF);
  ...
  do {
    ...
    UNS32 Size =
        (UNS32)(*pMappingParameter & (UNS32)0x000000FF); /* Size in bits */

    /* get variable only if Size != 0 and Size is lower than remaining bits in
     * the PDO */
    if (Size && ((offset + Size) <= 64)) {
      ...
      CopyBits((UNS8)Size, ((UNS8 *)tmp), 0, 0, (UNS8 *)&pdo->data[offset >> 3],
               (UNS8)(offset % 8), 0);
    ...
    if (offsetObjdict)
      while (offsetObjdict <= lastIndex) {
        ...
        case state4: /* Get Mapped Objects Number */
          ...
          pMappingCount = (UNS8 *)(d->objdict + offsetObjdict + numPdo)
                              ->pSubindex[0]
                              .pObject;
          numMap = 0;
          while (numMap < *pMappingCount) {
            ...
            UNS32 ByteSize;
            pMappingParameter = (UNS32 *)(d->objdict + offsetObjdict + numPdo)
                                    ->pSubindex[numMap + 1]
                                    .pObject;
            ...
            if (Size && ((offset + Size) <= (m->len << 3))) {
              /* copy bit per bit in little endian */
              CopyBits(Size, (UNS8 *)&m->data[offset >> 3], offset % 8, 0,
                       ((UNS8 *)tmp), 0, 0);
              /*1->8 => 1 ; 9->16 =>2, ... */
              ...
              if (objDict != OD_SUCCESSFUL) {
                MSG_ERR(0x1938, "error accessing to the mapped var : ",
                        numMap + 1);
              ...
              MSG_WAR(0x3942, "Variable updated by PDO cobid : ",
                      UNS16_LE(m->cob_id));
          ...
          if (d->RxPDO_EventTimers) {
            ...
            if (EventTimerDuration) {
              ...
              d->RxPDO_EventTimers[numPdo] =
                  SetAlarm(d, numPdo, d->RxPDO_EventTimers_Handler,
      ...
      while (offsetObjdict <= lastIndex) {
        ...
        case state4: /* check transmission type */
          ...
          /* RTR_SYNC means data prepared at SYNC, transmitted on RTR */
            ...
            if (d->PDO_status[numPdo].transmit_type_parameter &
                PDO_RTR_SYNC_READY) {
              ...
              MSG_ERR(0x1947, "Not ready RTR_SYNC TPDO send current data : ",
                      UNS16_LE(m->cob_id));
              status = state5;
            }
            break;
...
void CopyBits(UNS8 NbBits, UNS8 *SrcByteIndex, UNS8 SrcBitIndex,
  ...
  while (NbBits > 0) {
    ...
    UNS8 Mask = (UNS8)((0xff << (DestBitIndex + BitsToCopy)) |
                       (0xff >> (8 - DestBitIndex)));
...
UNS8 sendOnePDOevent(CO_Data *d, UNS8 pdoNum) {
  ...
  if (d->PDO_status[pdoNum].last_message.cob_id == pdo.cob_id &&
    ...
    if (EventTimerDuration) {
      ...
      d->PDO_status[pdoNum].event_timer = SetAlarm(
          d, pdoNum, &PDOEventTimerAlarm, MS_TO_TIMEVAL(EventTimerDuration), 0);
    }
    ...
    while (offsetObjdict <= lastIndex) {
      ...
      case state3:
        if (/* bSubCount always 5 with objdictedit -> check disabled */
            ...
            *(UNS32 *)d->objdict[offsetObjdict].pSubindex[1].pObject &
            0x80000000) {
        ...
        if (isSyncEvent && (*pTransmissionType >= TRANS_SYNC_MIN) &&
          ...
          MSG_WAR(0x306C, "  PDO is not on EVENT or synchro or not at this "
                          "SYNC. Trans type : ",
                  *pTransmissionType);
          status = state11;
        }
...
UNS32
TPDO_Communication_Parameter_Callback(CO_Data *d, const indextable *OD_entry,
                                      UNS8 bSubindex) {
  /* If PDO are actives */
  if (d->CurrentCommunicationState.csPDO)
    switch (bSubindex) {
    ...
    case 5: /* Changed event time */
    {
      ...
      UNS8 numPdo = (UNS8)(
          OD_entry - TPDO_com); /* number of the actual processed pdo-nr. */
      ...
      /* Call  PDOEventTimerAlarm for this TPDO, this will trigger emission et
       * reset timers */
      PDOEventTimerAlarm(d, numPdo);
      return 0;
    }
  ...
  if (offsetObjdict)
    while (offsetObjdict <= lastIndex) {
      ...
      if (errorCode == OD_SUCCESSFUL) {
        ...
        /* Transmission type */
        ptrTable->pSubindex[2].callback =
            &TPDO_Communication_Parameter_Callback;
        /* Inhibit time */
        ptrTable->pSubindex[3].callback =
            &TPDO_Communication_Parameter_Callback;
        /* Event timer */
        ptrTable->pSubindex[5].callback =
            &TPDO_Communication_Parameter_Callback;
      }
      pdoIndex++;
      offsetObjdict++;
